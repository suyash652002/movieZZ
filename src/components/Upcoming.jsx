...
const Upcoming = () => {
    ...
    useEffect(() => {
        const fetchApi = async () => {
            setLoading(true);
            const response = await fetch(`${url}movie/upcoming?language=en-US&page=1&api_key=${apiKey}`)
            if (!response.ok) {
                setLoading(false);
                setError(true);
                return;
    ...
    return (
        <>
            ...
            <div className=' bg-black h-[250px] lg:h-[500px]'>
                ...
                <h1 className='text-white font-bold lg:text-4xl text-center p-3'>Upcoming Movies</h1>
