...
const Cards = (props) => {
    return (
        <motion.div
            ...
            className='w-[100px]  lg:w-[250px]  flex-shrink-0 lg:text-md'>
            <img src={props.path} alt="popular movies" />
        </motion.div>
    );
}
...
const TopRated = () => {
    ...
    useEffect(() => {
        const fetchApi = async () => {
            setLoading(true);
            const response = await fetch(`${url}movie/top_rated?language=en-US&page=1&api_key=${apiKey}`)
            if (!response.ok) {
                setLoading(false);
                setError(true);
                return;
    ...
    return (
        <>
            ...
            <div className=' bg-black h-[250px] lg:h-[500px]'>
                ...
                <h1 className='text-white font-bold lg:text-4xl text-center p-3'>Top Rated Movies</h1>
